...
if($data['pageContent']['content']) {
	...
	echo '<div class="sk_content_wrap sk_contact">';
		foreach($data['pageContent']['content'] as $pageContent) {
			...
			echo '<div class="sk_content">';
				...
				if($pageContent['ImageName'] && file_exists(MEDIA_FILES_ROOT.'/content/thumb/'.$pageContent['ImageName'])){
					$contentImg = '<figure class="sk_img_right">
									<img src="'.MEDIA_FILES_SRC.'/content/thumb/'.$pageContent['ImageName'].'" alt="'.$pageContent['contentHeading'].'" />
								</figure>';
				}
